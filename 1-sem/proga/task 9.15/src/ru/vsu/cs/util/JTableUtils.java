...
public class JTableUtils {
    ...
    public static void writeArrayToJTable(JTable table, ArrayList<Integer> array) {
        ...
        for (int i=0; i<array.size(); i++) arr[i] = array.get(i).intValue();
        writeArrayToJTable(table, arr);
    }
     ...
     */
    public static ArrayList<Integer> readListFromJTable(JTable table) throws ParseException {
        try {
            return ArrayUtils.toList(readArrayFromJTable(table, Integer.class, Integer::parseInt, false, 0));
        } catch (JTableUtilsException impossible) {
        }
        return null;
    }
